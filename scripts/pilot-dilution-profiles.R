...
pilot_dilution_profiles <- function(fpath, outpath, dim = NA){
	...
	dt[4:ncol(dt)] <- suppressWarnings(apply(dt[4:ncol(dt)], 2, function(x) as.numeric(x)))
	...
	dt <- dplyr::summarise(dt, mean_sal = mean(sal, na.rm = TRUE), sd_sal = sd(sal, na.rm = TRUE), n = length(sal))
	dt$se <- 2 * (dt$sd / sqrt(dt$n))
	...
	gg <- gg + ggplot2::geom_point(size = 3) + ggplot2::geom_errorbar(ggplot2::aes(ymin = lower, ymax = upper), width = 0.3, color = "black")
	gg <- gg + ggplot2::geom_smooth(se = FALSE, color = "black")
	gg + ggplot2::ylab("Salinity") +	ggplot2::xlab("Days") + theme_opts
